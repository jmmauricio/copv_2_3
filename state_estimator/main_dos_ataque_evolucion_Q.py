...
col = plot_tools.set_style(plt)*3
col = col[:df_evol_Q.shape[1]]
...
for subplot_idx in range(num_subplots):
    ...
    off_val = np.round(df_evol_Q.shape[1]/2 - 0)
    offsets = np.linspace(-width*off_val, width*off_val, len(col))  # Espaciado para las columnas
