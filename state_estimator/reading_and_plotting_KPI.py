...
for c in fp_list:
    ...
    for hour in ['08', '10', '13']:
        ...
        x_values = [str(item) for item in np.linspace(0.01, 25, num_lmb)]
        ...
        for x in x_values:            
            ...
            TP = data[c][hour][x][tipo]['TP']
            TN = data[c][hour][x][tipo]['TN']
            FP = data[c][hour][x][tipo]['FP']
            FN = data[c][hour][x][tipo]['FN']
            
            si_infinito = 0
            
            pre = [item[0]/(item[0] + item[1]) if item[0] + item[1] != 0 else si_infinito for item in zip(TP, FP)]
            re = [item[0]/(item[0] + item[1])  if item[0] + item[1] != 0 else si_infinito for item in zip(TP, FN)]
            ac = [(item[0] + item[1])/(item[0] + item[1] + item[2] + item[3])  if item[0] + item[1] != 0 else si_infinito for item in zip(TP, TN, FP, FN)]
            
            precision.append([np.mean([p for p in pre if p != None]), np.std([p for p in pre if p != None]), np.max([p for p in pre if p != None])])
            recall.append([np.mean([r for r in re if r != None]), np.std([r for r in re if r != None]), np.max([r for r in re if r != None])])
            accuracy.append([np.mean([a for a in ac if a != None]), np.std([a for a in ac if a != None]), np.max([a for a in ac if a != None])])
        ...
        axs[i,j+1].plot(lmb_range, [item[0]*100 for item in precision], 'b', label='avg precision')
        axs[i,j+1].plot(lmb_range, [item[0]*100 for item in recall], 'r', label='avg recall')
        axs[i,j+1].plot(lmb_range, [item[0]*100 for item in accuracy], 'k', label='avg accuracy')
        # axs[i,j+1].plot(lmb_range, [(item[0] + item[1])*100 for item in precision], 'b--')
        # axs[i,j+1].plot(lmb_range, [(item[0] + item[1])*100 for item in recall], 'r--')
        # axs[i,j+1].plot(lmb_range, [(item[0] + item[1])*100 for item in accuracy], 'k--')
        # axs[i,j+1].plot(lmb_range, [(item[0] - item[1])*100 for item in precision], 'b--')
        # axs[i,j+1].plot(lmb_range, [(item[0] - item[1])*100 for item in recall], 'r--')
        # axs[i,j+1].plot(lmb_range, [(item[0] - item[1])*100 for item in accuracy], 'k--')
