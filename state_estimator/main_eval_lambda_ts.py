...
for c in case:
    for hour in ['08', '09', '10', '11', '12', '13', '14']:
        for minute in ['00', '15', '30', '45']:
            ...
            # Barremos lambda:
            Results_Huber_lmb = []
            lmb_range = np.linspace(0.01, 8, 100)
            ...
            minimos_U.append((lmb_range[min_index], error_U_Huber[min_index]))
            
            
            min_value = np.min(error_ang_Huber)
            min_index = list(error_ang_Huber).index(min_value)
            minimos_ang.append((lmb_range[min_index], error_ang_Huber[min_index]))
         
            
for item in minimos_U:         
    ax[0].plot(item[0], item[1], color=col[0], marker = "*")
for item in minimos_ang:         
    ax[1].plot(item[0], item[1], color=col[0], marker = "*")
