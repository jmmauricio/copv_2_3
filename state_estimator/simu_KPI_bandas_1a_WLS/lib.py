...
class grid:
    ...
    def state_estimation(self, tol = 1e-6, niter = 100, Huber = False, lmb = None, rn = False, print_info = True):
        ...
        if print_info:
            ...
            print('')
        Results = {'solution': [], 'residual': [], 'jacobian': [], 'Q': [], 'std_sol': None, 'max_res': None, 'rm_meas': [], 'pop': []}
        while (cond):
...
def identification_fun(lmb_min, lmb_max, lmb_num, n_simus, names, net_base, bandas):
...
def detection(lmb_min, lmb_max, lmb_num, n_simus, names, net_base, bandas):
    ...
    for lmb_value in lmb_range:
        for names_ataque in [names_P, names_Q, names_U]:            
            for _ in range(n_simus):     
                ...
                # Corremos el estimador de estado
                Results_WLS = net.state_estimation(tol = 1e-4, niter = 50, Huber = False, lmb = lmb_value, rn = True)                
                result_rows = Results_WLS['pop']
...
def detection_n(lmb_min, lmb_max, lmb_num, n_simus, names, net_base, bandas, num_attacks):
    ...
    for lmb_value in lmb_range:
        ...
        for _ in range(n_simus):     
            ...
            # Contamos
            if Res == '0' or Res == '1' or Res == '2':
                count[str(lmb_value)]['Detection'].append(True)
            else:
                count[str(lmb_value)]['Detection'].append(False)
    return count
