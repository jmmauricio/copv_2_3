...
for index_bandas, bandas in enumerate(bandas_array): 
    ...
    for c in ['090neg', '090pos']:  
        ...
        for hour in ['08', '10', '13']: 
            ...
            count_sinMT = lib.detection(n_simus = n_simus, names = names, net_base = net_sinMT, bandas = bandas)
            count_conMT = lib.detection(n_simus = n_simus, names = names, net_base = net_conMT, bandas = bandas)
            count_sinMT_conPSM = lib.detection(n_simus = n_simus, names = names, net_base = net_sinMT_conPSM, bandas = bandas)
    ...
    # Saving results
    with open('results/detection_sinMT_' + str(index_bandas) + '.json', 'w') as json_file:
        json.dump(resultados_sinMT, json_file, indent=4)
    with open('results/detection_conMT_' + str(index_bandas) + '.json', 'w') as json_file:
        json.dump(resultados_conMT, json_file, indent=4)
    with open('results/detection_sinMT_conPSM_' + str(index_bandas) + '.json', 'w') as json_file:
