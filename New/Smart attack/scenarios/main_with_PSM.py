...
for percent_attacked in [15, 30, 60, 75]:
    for scenario in range(10):
        ...
        while n_simu < 20:
            ...
            match scenario:
                case 0:        
                    # Disparo inversor -> Ataque inversor -> U_atacada < U_medida -> LV0103: 0.98 por su valor
                    node = 5
                    nmeas = 6
                    amplitude = 0.98
                    minute = '00'
            ...
            for meas_index in meas_safe:
                net.meas[meas_index].std = 100
                meas[meas_index]['std'] = 100
            ...
            for meas_index in meas_attacked:
                net.meas[meas_index].std = std_pqui[meas[meas_index]['type']]
                # meas[meas_index]['std'] = std_pqui[meas[meas_index]['type']]
            
                    
            # Imposing the main parameter of the attack
            id_ref = net.constrained_meas[-1].ref + 1
            net.constrained_meas.append(lib.measurement(id_ref, node, None, 'U', amplitude*net_clean.nodes[node].V, 0, net.nodes, net.lines, net.n))
            meas[nmeas]['value'] = amplitude*net_clean.nodes[node].V
            net.meas[nmeas].value = amplitude*net_clean.nodes[node].V
            ...
            if len(solution['solution']) != 0:
                ...
                optimal_index = 0
                for item in zip(meas, solution['z_output']):
                    print(f"{item[0]['value']:.3f} --> {item[1]:.3f}")
                    optimal_index += (item[0]['value'] - item[1])**2
                    
                    
                # Assigning the new values to meas
                for meas_index in meas_attacked:
                    meas[meas_index]['value'] = solution['z_output'][meas_index]
                
                print(optimal_index)
                 
                # Saving the attack
                with open(f'with PSM/case_{scenario}_percent_{percent_attacked}_index_{int(optimal_index)}_{int((optimal_index-int(optimal_index))*1000)}.json', 'w') as json_file:
                        json.dump(meas, json_file, indent=4)
