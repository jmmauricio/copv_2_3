...
class grid:
    def __init__(self, nodes=None, lines=None, meas=None, constraints=None, path_topology=None, path_measurements=None, MT=True, PSM=False):
                ...
                for line_con in line.nodes:
                    if node.ref != line_con.ref:
                        if line.Transformer == False:
                            self.Y[node.ref, line_con.ref] = -1/line.Z
            ...
            for line in node.lines:
                for line_con in line.nodes:
                    if node.ref != line_con.ref:
                        ...
                        line.G = np.real(self.Y[node.ref, line_con.ref])
                        line.B = np.imag(self.Y[node.ref, line_con.ref])
        # Assigning G and B to the nodes
        for node in self.nodes:
            # node.G = np.real(self.Y[node.ref - 1, node.ref - 1])
            # node.B = np.imag(self.Y[node.ref - 1, node.ref - 1])
            node.G = np.real(self.Y[node.ref, node.ref])
            node.B = np.imag(self.Y[node.ref, node.ref])
        return self.Y        
    ...
    def smart_attack_objective(self, x):
        self.assign(x)
        objective = np.sum([((m.value - m.h())/m.std)**2 for m in self.meas])
        return objective
...
def identification_fun(n_simus, names, net_base, bandas, n_attacks):
    # Data
    lmb_value = 2.5
    num_attacks = n_attacks
        
    # Results
    count = {'Precision': 0, 'Accuracy': 0, 'Recall': 0, 'F1': 0, 'norm2': 0, 'norminf': 0, 'z': 0}
    ...
    for _ in range(n_simus):     
        ...
        count['Precision'] += Precision
        count['Recall'] += Recall
        count['Accuracy'] += Accuracy
        count['F1'] += F1
        count['norm2'] += np.linalg.norm(x_clean - x_Huber, 2)/len(x_clean)
        count['norminf'] += np.linalg.norm(x_clean - x_Huber, np.inf)
        count['z'] += z
        
    
    
    count['Precision'] = count['Precision']/(n_simus)
    count['Recall'] = count['Recall']/(n_simus)
    count['Accuracy'] = count['Accuracy']/(n_simus)  
    count['F1'] = count['F1']/(n_simus)  
    count['norm2'] = count['norm2']/(n_simus)  
    count['norminf'] = count['norminf']/(n_simus)  
