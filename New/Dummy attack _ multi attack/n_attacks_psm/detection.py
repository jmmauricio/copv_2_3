...
for index_bandas, bandas in enumerate(bandas_array): 
    ...
    for c in ['090neg', '090pos']:  
        ...
        for hour in ['08', '10', '13']: 
            resultados_sinMT[c][hour] = dict()
            resultados_conMT[c][hour] = dict()
            resultados_sinMT_conPSM[c][hour] = dict()
            for n in range(2, 11):
                ...
                count_sinMT = lib.detection(n_simus = n_simus, names = names, net_base = net_sinMT, bandas = bandas, n_attacks = n)
                count_conMT = lib.detection(n_simus = n_simus, names = names, net_base = net_conMT, bandas = bandas, n_attacks = n)
                count_sinMT_conPSM = lib.detection(n_simus = n_simus, names = names, net_base = net_sinMT_conPSM, bandas = bandas, n_attacks = n)
                
                resultados_sinMT[c][hour][n] = count_sinMT
                resultados_conMT[c][hour][n] = count_conMT
                resultados_sinMT_conPSM[c][hour][n] = count_sinMT_conPSM
            
    # Saving results
    with open('results/detection_sinMT_banda' + str(index_bandas) + '.json', 'w') as json_file:
        json.dump(resultados_sinMT, json_file, indent=4)
    with open('results/detection_conMT_banda' + str(index_bandas) + '.json', 'w') as json_file:
        json.dump(resultados_conMT, json_file, indent=4)
    with open('results/detection_sinMT_conPSM_banda' + str(index_bandas) + '.json', 'w') as json_file:
