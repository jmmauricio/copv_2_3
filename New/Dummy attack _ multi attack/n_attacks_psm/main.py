...
for index_bandas, bandas in enumerate(bandas_array): 
    ...
    for c in ['090neg', '090pos']:  
        ...
        for hour in ['08', '10', '13']: 
            ...
            for n in range(2, 11):
                ...
                count_sinMT = lib.identification_fun(n_simus = n_simus, names = names, net_base = net_sinMT, bandas = bandas, n_attacks = n)
                count_conMT = lib.identification_fun(n_simus = n_simus, names = names, net_base = net_conMT, bandas = bandas, n_attacks = n)
                count_sinMT_conPSM = lib.identification_fun(n_simus = n_simus, names = names, net_base = net_sinMT_conPSM, bandas = bandas, n_attacks = n)
    ...
    # Saving results
    with open('results/sinMT_banda' + str(index_bandas) + '.json', 'w') as json_file:
        json.dump(resultados_sinMT, json_file, indent=4)
    with open('results/conMT_banda' + str(index_bandas) + '.json', 'w') as json_file:
        json.dump(resultados_conMT, json_file, indent=4)
    with open('results/sinMT_conPSM_banda' + str(index_bandas) + '.json', 'w') as json_file:
